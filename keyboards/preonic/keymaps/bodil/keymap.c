...
[_UNICODE] = LAYOUT_preonic_grid(
  KC_GRV,  _______, _______, _______, X_CASH,  _______, _______, _______, _______, _______, _______, KC_DEL,
  KC_LEAD, _______, _______, X_AE,    _______, X_THORN, _______, _______, _______, X_OE,    _______, _______,
  COMPOSE, X_AA,    X_SS,    X_ETH,   X_FORALL,_______, _______, _______, X_DASH,  X_LMBD,  _______, _______,
 ...
 * |------+------+------+------+------+------+------+------+------+------+------+------|
 * |      |MBtn1 |MouseU|MBtn2 |WheelU|      |      |      |      |      |      |PrtSc |
 * |------+------+------+------+------+------+------+------+------+------+------+------|
...
[_FUNCTION] = LAYOUT_preonic_grid(
  ...
  _______, KC_BTN1, KC_MS_U, KC_BTN2, KC_WH_U, _______, _______, _______, _______, _______, _______, KC_PSCR,
...
bool process_record_user(uint16_t keycode, keyrecord_t* record) {
    // Ctrl+Lower+Raise (in that order) sends Ctrl+Alt+Del.
    if (record->event.pressed && (keycode == RAISE)) {
        if ((get_mods() & MOD_BIT(KC_LCTL)) == MOD_BIT(KC_LCTL) && IS_LAYER_ON(_UNICODE)) {
            tap_code16(A(KC_DEL));
            return false;
        }
    }
...
#define SEQ_SINGLE_ACCENT(keycode, acc_code, upper, lower)                  \
    SEQ_TWO_KEYS(keycode, acc_code) {                                       \
        send_unicode_string((get_mods() & MOD_MASK_SHIFT) ? upper : lower); \
    }
